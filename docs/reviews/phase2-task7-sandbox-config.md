...
|---|---------|----------------|
| S1 | Seccomp filters absent | Design doc mentions them for Linux. File follow-up issue. |
| S2 | No `--unshare-pid` | Standard bwrap hardening measure. |
| S3 | ReadDeny mapped to `--tmpfs` | Document this design choice (hides content vs permission denied). |
| S4 | `expandPath` error handling | Silently swallows `os.UserHomeDir()` errors. Log warning. |
| S5 | No `binaryPath` validation | Empty string or spaces produce broken args. |
| S6 | Write profile to file | Avoids argument-length limits and shell-escaping. |
| S7 | Container tier returns nil silently | Could mask configuration errors. |
