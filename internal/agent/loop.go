...
func validateMode(mode types.ScannerMode, name string) error {
	...
	if !mode.Valid() {
		return sigilerr.Errorf(sigilerr.CodeAgentLoopInvalidInput, "invalid ScannerModes."+name+": %q", mode)
	}
	return nil
}
	...
	for i := range maxToolLoopIterations {
		...
		if text != "" {
			...
			if scanErr != nil {
				...
				l.auditOutputBlocked(ctx, msg, intermediateThreat, auditReason)
				return "", nil, scanErr
			}
			text = scannedText
...
func (l *Loop) auditOutputBlocked(ctx context.Context, msg InboundMessage, threatInfo *store.ThreatInfo, reason string) {
	...
	details := map[string]any{
		"stage": string(types.ScanStageOutput),
	}
	if threatInfo == nil {
		details["scanner_error"] = true
	} else if threatInfo.Detected {
	...
	entry := &store.AuditEntry{
		ID:          uuid.New().String(),
		Timestamp:   time.Now().UTC(),
		Action:      "agent_loop.output_blocked",
		Actor:       msg.UserID,
		WorkspaceID: msg.WorkspaceID,
		SessionID:   msg.SessionID,
		Details:     details,
...
func (l *Loop) auditToolScan(ctx context.Context, msg InboundMessage, toolCallID, toolName string, threatInfo *store.ThreatInfo, scanErr error) {
	...
	if !threatInfo.Detected && !threatInfo.Bypassed && scanErr == nil {
		return
	}
...
func (l *Loop) scanContent(ctx context.Context, content string, stage types.ScanStage, origin types.Origin, mode types.ScannerMode, scannerFailCount *int, logAttrs ...slog.Attr) (string, *store.ThreatInfo, error) {
	...
	if scanErr != nil {
		...
		if sigilerr.HasCode(scanErr, sigilerr.CodeSecurityScannerContentTooLarge) {
			...
			slog.LogAttrs(ctx, slog.LevelError, "content exceeds scanner limit",
				append([]slog.Attr{
